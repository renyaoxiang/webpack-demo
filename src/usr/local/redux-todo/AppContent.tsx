...
export class AppContent2 extends Component<any, any> {
	...
	public render() {
		...
		return <div>
			<div>
				<button onClick={this.doCreate.bind(this)} disabled={readOnly}>
					create
				</button>
			...
			<input type='input' disabled={readOnly} ref={ref => this.input = ref}
			...
			<ul>
				{this.props.model.todos.map(it => {
					return <li key={it.id}  >
						...
						<button onClick={() => this.remove(it.id)}
							disabled={readOnly}>remove</button>
						<button onClick={() => this.toggle(it.id)}
							disabled={readOnly}>toggle</button>
						|{it.id}|{it.text}|
					</li>
				})}
			</ul>
...
export const AppContent = connect(state => ({
	model: {
		...state.todo,
