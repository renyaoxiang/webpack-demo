			...
			<div>
				...
				<ul>
					{this.props.model.todos.map(it => {
						return (
							<li key={it.id}>
								<span style={{ display: "inline-block", width: "50px" }}>{it.complete.toString()}</span>
								<button onClick={() => this.remove(it.id)} disabled={readOnly}>
									remove
								</button>
								<button onClick={() => this.toggle(it.id)} disabled={readOnly}>
									toggle
								</button>
								|{it.id}|{it.text}|
							</li>
